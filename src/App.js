...
function App() {
  ...
  const [listBarStatus,setListBarStatus]=React.useState("all")
  const [listOfListBar,setListOfListBar] = React.useState(stocklist)
  const [timeFrame, setTimeFrame] = React.useState("1wk")
  const [symbol, setSymbol] = React.useState("RELIANCE")
  const [showTrendlines,setShowTrendlines] = React.useState(true)
  const [candlePatternList, setCandlePatternList] = React.useState([])
  const [chartPatternList, setChartPatternList] = React.useState([])
  const [stockData, setStockData] = React.useState({candleData:[
...
trendlineData:[]})
  ...
  function updateShowTrendlines(){
    setShowTrendlines(oldState=>!oldState)
  }

  function updateListOfListBar(param){
    setListBarStatus(param)
    if(param === "all"){
      setListOfListBar(stocklist)
    }
    else if("triangletrendline".includes(param)){
      setListOfListBar(chartPatternList.filter(item=>item.Pattern.includes(param)))
    }else if("dojistarhammerengulphing".includes(param)){
      setListOfListBar(candlePatternList.filter(item=>item.Pattern.includes(param)))
    }
  }

  async function fetchCandlePatternList(){
          let res = await fetch("./Data/drawtrendlines/"+timeFrame+"/CandlePattern.csv")
          let responseText = await res.text()
          let data = responseText.split('\n').map((row) => {
                        const [Symbol,Pattern]=row.split(',');
                        return {Symbol: Symbol,Pattern: Pattern};})
          data.shift()
          data.pop()
          setCandlePatternList(data)
                      }

  async function fetchChartPatternList(){
          let res = await fetch("./Data/drawtrendlines/"+timeFrame+"/ChartPattern.csv")
          let responseText = await res.text()
          let data = responseText.split('\n').map((row) => {
                const [Symbol,Pattern]=row.split(',');
                return {Symbol: Symbol,Pattern: Pattern};})
          data.shift()
          data.pop()
          setChartPatternList(data)
          }

  async function fetchStockData(){
    let stockdata = []
    let trendlinedata = []
          let res = await fetch("./Data/"+timeFrame+"/"+symbol+".csv")
          let responseText = await res.text()
          stockdata = responseText.split('\n').map(row => {
          ...
          stockdata.shift()
          stockdata.pop()
          let resl = await fetch("./Data/drawtrendlines_/"+timeFrame+"/"+symbol+".csv")
          let responselText = await resl.text()
          if(responselText.length != 410){
          trendlinedata = responselText.split('\n').map(row => {
                          const [x1,y1,x2,y2]=row.split(',');
                          if(x1 && x1!="x1"){const [Year_1,Month_1,Day_1]=x1.split('-');
                          const [Year_3,Month_3,Day_3]=x2.split('-');
                          if(Month_1){return [{
                            time: {year: Year_1*1, month: Month_1*1, day: (Day_1[0]+Day_1[1])*1},
                            value: y1 * 1,
                          },
                          {
                            time: {year: Year_3*1, month: Month_3*1, day: (Day_3[0]+Day_3[1])*1},
                            value: y2 * 1,
                          }];}}})
            trendlinedata.shift()
            trendlinedata.pop()}
       setStockData({candleData:stockdata,trendlineData:trendlinedata})
      }

  React.useEffect(() => 
    {
     fetchStockData()
     fetchCandlePatternList()
     fetchChartPatternList()
    },[timeFrame,symbol,listOfListBar])

  React.useEffect(()=>{
    
    if(listBarStatus === "all"){
      setListOfListBar(stocklist)
     }else if(listBarStatus === "chartpattern"){
       setListOfListBar(chartPatternList)
     }else if(listBarStatus === "candlepattern"){
       setListOfListBar(candlePatternList)
     }
  },[timeFrame])
    
    function fetchStocksList(){
      fetch("./Data/NSE_STOCKS.csv").then(response=>response.text()).then(responseText=>{
      setStocklist( responseText.split('\n').map((row) => {
                    const [id,symbol,name,cap]=row.split(',');
                    return {Symbol: symbol,StockName: name};})
                  )
      setListOfListBar( responseText.split('\n').map((row) => {
                    const [id,symbol,name,cap]=row.split(',');
                    return {Symbol: symbol,StockName: name};})
                  )})
      }
  ...
  return (
          <div id="main-container">
            <Navbar
              timeFrame={timeFrame}
              Symbol = {symbol}
              O={stockData.candleData[stockData.candleData.length-1].open}
              H={stockData.candleData[stockData.candleData.length-1].high}
              L={stockData.candleData[stockData.candleData.length-1].low}
              C={stockData.candleData[stockData.candleData.length-1].close}
              setTimeFrame={setTimeFrame}/>
            <div id="second-container">
              <Sidebar
                updateShowTrendlines={updateShowTrendlines}
                updateListOfListBar={updateListOfListBar}/>
              <ChartComponent
                timeFrame={timeFrame}
                Symbol={symbol}
                data={stockData}
